...
class dsd extends \core_external\external_api {
    ...
    public static function execute($ano_lectivo) {
        ...
        $query = "SELECT /*+ MAX_EXECUTION_TIME(0) */
                         ...
                         courseCD AS 'crs',
	                     usrID AS 'usr',
                         to_ascii(usrName) AS 'usrnm'
                  FROM (
                        SELECT /*+ MAX_EXECUTION_TIME(0) */
                               CONCAT(usr.id, crs.id) AS 'id',
                               '" . $ano_lectivo . "' AS ano_lectivo,
                               CONCAT(LEFT (crs.idnumber, 5), usr.username) AS usr_UC,
                               LEFT(crs.idnumber, 5) AS courseCD,
                               usr.username AS usrID,
                               CONCAT(usr.firstname, ' ', usr.lastname) AS usrName
    ...
    public static function execute_returns() {
        return new external_multiple_structure(
            new external_single_structure([
                ...
                'usr' => new external_value(PARAM_TEXT, 'username do docente/tutor'),
                'usrnm' => new external_value(PARAM_TEXT, 'nome do docente/tutor'),
            ])
        );
    }
}
