...
class estudantes_1C_AC_opcao extends \core_external\external_api {
    ...
    public static function execute($ano_lectivo) {
        ...
        $query = "SELECT /*+ MAX_EXECUTION_TIME(0) */
                  ...
                  FROM mdl_course crs
                      ...
                      INNER JOIN mdl_choice chc ON chc.course = crs.id
                      INNER JOIN mdl_choice_options chcOpt ON chcOpt.choiceid = chc.id
                      INNER JOIN mdl_choice_answers chcAsw ON (chcAsw.choiceid = chc.id AND chcAsw.optionid = chcOpt.id AND chcAsw.userid = std.id)
                  WHERE (crs.idnumber LIKE CONCAT('_1___\_', MID('" . $ano_lectivo . "', 3, 2), '\___')
                          AND RIGHT(crs.idnumber, 2) REGEXP('^[0-9]+$')
                          AND SUBSTRING(crs.idnumber, 10, 2) <> '00')
                      AND e.enrol IN ('database','manual')
                      AND ue.status = 0
                      AND chc.name = 'Decisão sobre a Avaliação'
                      AND chcOpt.text = 'Avaliação Contínua'
