...
class estudantes_1C_efolios extends \core_external\external_api {
    ...
    public static function execute($ano_lectivo) {
        ...
        $query = "SELECT /*+ MAX_EXECUTION_TIME(0) */
                         ...
                         COUNT(DISTINCT std.id) AS 'tsubs'
                  FROM mdl_course crs
                      ...
                      INNER JOIN mdl_grade_items grd ON grd.courseid = crs.id
                      INNER JOIN mdl_grade_grades stdgrd ON (stdgrd.itemid = grd.id AND stdgrd.userid = std.id)
                  WHERE (crs.idnumber LIKE CONCAT('_____\_', MID('" . $ano_lectivo . "', 3, 2), '\___')
                          AND RIGHT(crs.idnumber, 2) REGEXP('^[0-9]+$')
                          AND SUBSTRING(crs.idnumber, 10, 2) <> '00')
                      AND e.enrol IN ('database','manual')
                      AND ue.status = 0
                      AND grd.itemname = 'E-fólios'
                      AND stdgrd.finalgrade > 0.00000
                  GROUP BY LEFT (crs.idnumber, 5)
                  ORDER BY std.username;";
    ...
    public static function execute_returns() {
        return new external_multiple_structure(
            new external_single_structure([
                ...
                'tsubs' => new external_value(PARAM_INT, 'total de submissoes'),
            ])
        );
    }
}
