...
class posts_UC extends \core_external\external_api {
    ...
    public static function execute($ano_lectivo) {
        ...
        $query = "SELECT /*+ MAX_EXECUTION_TIME(0) */
                         ...
                         COUNT(DISTINCT forPst.id) AS 'tpost'
                  FROM mdl_course crs
                      INNER JOIN mdl_forum_discussions forDsc ON forDsc.course = crs.id
                      INNER JOIN mdl_forum_posts forPst ON forPst.discussion = forDsc.id
