...
class posts_UC_palavras_docentes_tutores extends \core_external\external_api {
    ...
    public static function execute($ano_lectivo) {
        ...
        $query = "SELECT /*+ MAX_EXECUTION_TIME(0) */
                         ...
                         SUM(LENGTH(forPstTxt) - LENGTH(REPLACE(CAST(forPstTxt AS CHAR CHARACTER SET utf8), ' ', '')) + 2) AS 'tword'
                  FROM moodle.mv_CoursePostAll
                  WHERE (courseCD LIKE CONVERT(CONCAT('_____\_', MID(" . $ano_lectivo . ", 3, 2), '\___') USING utf8)
                      ...
                      AND (forPstDate >= UNIX_TIMESTAMP('" . $fromDt . "')
	                      AND forPstDate <= UNIX_TIMESTAMP('" . $toDt . "'))
                      AND (roleID = 3 OR roleID = 27 OR roleID = 10)
    ...
    public static function execute_returns() {
        return new external_multiple_structure(
            new external_single_structure([
                ...
                'tword' => new external_value(PARAM_INT, 'total de palavras'),
            ])
        );
    }
}
