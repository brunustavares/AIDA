...
class posts_UC_utilizadores extends \core_external\external_api {
    ...
    public static function execute($ano_lectivo) {
        ...
        $query = "SELECT /*+ MAX_EXECUTION_TIME(0) */
                         VIEW_ID() AS 'id',
                         '" . $ano_lectivo . "' AS 'al',
                         LEFT (crs1.idnumber, 5) AS 'crs',
                         tPst.usrPT AS 'tpart'
                  FROM mdl_course crs1
                      RIGHT JOIN (
                                  SELECT crs2.id AS crsID,
                                         COUNT(DISTINCT forPst.userid) AS usrPT,
                                         COUNT(DISTINCT forPst.id) AS nPst
                                  FROM mdl_forum_posts forPst
                                      INNER JOIN mdl_forum_discussions forDsc ON forDsc.id = forPst.discussion
                                      INNER JOIN mdl_course crs2 ON crs2.id = forDsc.course
                                  WHERE (forPst.created >= UNIX_TIMESTAMP('" . $fromDt . "')
                                      AND forPst.created <= UNIX_TIMESTAMP('" . $toDt . "'))
                                  GROUP BY crs2.id
                                 ) tPst ON tPst.crsID = crs1.id
                  WHERE (crs1.idnumber LIKE CONVERT(CONCAT('_____\_', MID(" . $ano_lectivo . ", 3, 2), '\___') USING utf8)
                          AND RIGHT(crs1.idnumber, 2) REGEXP('^[0-9]+$')
                          AND crs1.idnumber NOT LIKE '%_00')
                  GROUP BY LEFT (crs1.idnumber, 5)
                  ORDER BY crs1.idnumber;";
    ...
    public static function execute_returns() {
        return new external_multiple_structure(
            new external_single_structure([
                ...
                'tpart' => new external_value(PARAM_INT, 'total de participantes'),
            ])
        );
    }
}
